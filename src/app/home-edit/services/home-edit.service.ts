...
import { Locale, Documents, IHomePage, Collections, PAGENAME } from 'src/app/shared/global.model';
...
export class HomeEditService {
  ...
  initializeHomeEditPageContent(): Observable<IHomePage | undefined> {
    return this.contentService.getContentForPage<IHomePage>(PAGENAME.HOME);
  }
  ...
  onSubmit(value: IHomePage): Observable<void> {
    return this.contentService.getApplicationLocale().pipe(
      switchMap((locale) => {
        ...
        return this.datastore.collection<IHomePage>(Collections.HOME).doc(document).update(value);
      })
    );
  }
